...
# OctaveConv2D tests
def test_octave_conv2d_padding_output_shape():
    # verify output_shape with padding
    kwargs = {"filters": 3, "kernel_size": 3, "low_freq_ratio": 0.5, "padding": "same"}
    layer = OctaveConv2D(**kwargs)
    y = layer(np.ones((2, 28, 28, 3)))
...
def test_octave_conv2d_dilation_rate_output_shape():
...
def test_octave_conv2d_strides_output_shape():
    # verify output_shape with strides
    kwargs = {
        "filters": 3,
        "kernel_size": 3,
...
def test_octave_conv2d_regularizers():
    # verify regularizers
    kwargs = {
        "filters": 3,
        "kernel_size": 3,
    ...
    assert len(layer.losses) == 4
    layer(np.ones((1, 10, 10, 4)))
    assert len(layer.losses) == 8


def test_octave_conv2d_constraints():
    # verify contraints
    def identity(x):
        return x
...
def test_ocatve_conv2d_output():
    ...
    layer = OctaveConv2D(**kwargs)
    y = layer(np.ones((1, 2, 2, 2)))
    assert len(y) == 2
    np.testing.assert_allclose(
        [[[[8.0, 8.0], [8.0, 8.0]], [[8.0, 8.0], [8.0, 8.0]]]], y[0]
    )
    np.testing.assert_allclose([[[[2.0]]]], y[1])


def test_octave_conv2d_serialization():
    # verify serialization
    kwargs = {"filters": 3, "kernel_size": 3, "low_freq_ratio": 0.5, "padding": "same"}
    layer = OctaveConv2D(**kwargs)
    serialized_layer = tf.keras.layers.serialize(layer)
...
# OctaveConv3D tests
def test_octave_conv3d_padding_output_shape():
    # verify output_shape with padding
    kwargs = {"filters": 3, "kernel_size": 3, "low_freq_ratio": 0.5, "padding": "same"}
    layer = OctaveConv3D(**kwargs)
    y = layer(np.ones((2, 28, 28, 28, 3)))
    assert (y[0].shape.as_list(), y[1].shape.as_list()) == (
        [2, 28, 28, 28, 2],
        [2, 14, 14, 14, 1],
    )
...
def test_octave_conv3d_dilation_rate_output_shape():
    # verify output_shape with dilation_rate
    kwargs = {
        "filters": 3,
        "kernel_size": 3,
...
def test_octave_conv3d_strides_output_shape():
    # verify output_shape with strides
    kwargs = {
        "filters": 3,
        "kernel_size": 3,
...
def test_octave_conv3d_regularizers():
    # verify regularizers
    kwargs = {
        "filters": 3,
        "kernel_size": 3,
    ...
    assert len(layer.losses) == 4
    layer(np.ones((1, 10, 10, 10, 4)))
    assert len(layer.losses) == 8


def test_octave_conv3d_constraints():
    # verify constraints
    def identity(x):
        return x
...
def test_octave_conv3d_serialization():
    # verify serialization
    kwargs = {"filters": 3, "kernel_size": 3, "low_freq_ratio": 0.5, "padding": "same"}
    layer = OctaveConv3D(**kwargs)
    serialized_layer = tf.keras.layers.serialize(layer)
